...
async def wait_strobe(pin, timeout_ms=50):
...
async def read_byte_non_blocking():
    ...
    if not await wait_strobe(strobe_high, timeout_ms=50):
        return None
    high = read_nibble()
    ...
    if not await wait_strobe(strobe_low, timeout_ms=50):
        return None
    low = read_nibble()
...
# =====================================================================
# SENDER (Active-LOW Pulse)
# =====================================================================

async def send_nibble(nibble, strobe_pin):
    """Schreibt ein Nibble und erzeugt einen LOW-Puls."""
    for i in range(4):
...
async def main():
    print("RP2040 Bus aktiv (Pins 10-13 Data, 14-15 Strobe, Pull-Up).")
